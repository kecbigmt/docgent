	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(chatModel *MockChatModel, conversationService *MockConversationService, fileQueryService *MockFileQueryService, fileChangeService *MockFileChangeService, proposalRepository *MockProposalRepository, ragCorpus *MockRAGCorpus) {
				...
				// 2回目のメッセージ：ファイルを更新
				chatModel.On("SendMessage", mock.Anything, mock.Anything).Return(`<modify_file><path>docs/api.md</path><hunk><search>エンドポイント</search><replace>endpoint</replace></hunk></modify_file>`, nil).Once()
				fileChangeService.On("ModifyFile", mock.Anything, "docs/api.md", mock.Anything).Return(nil)

				// 3回目のメッセージ：タスクを完了
				chatModel.On("SendMessage", mock.Anything, mock.Anything).Return(`<attempt_complete><message>提案を更新しました</message></attempt_complete>`, nil).Once()
